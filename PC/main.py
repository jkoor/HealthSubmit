...
def login_web(account, password, url):
    ...
    try:
        if a.text[:10] == "layer.open":
            result = xh + (a.text[22:][:-54])
            return False
        else:
            # print(xh + "登录成功")
            return True
    except:
        result = str(account["xh"]) + "用户名或者密码错误，请重新输入!"
        return False
...
def get_private_data():
    ...
    try:
        ...
        a = soup.find(attrs={"type": "text/javascript"})
        if a.text[:10] == "layer.open":
            result = str(account["xh"]) + (a.text[22:][:-92])
        else:
            result = astr(account["xh"]) + "填报失败"
        return False
...
def submit_healthcondition():
    ...
    try:
        ...
        if a.text[:10] == "layer.open":
            ...
            result = str(account["xh"]) + "填报失败，请重试"
    except:
        result = str(account["xh"]) + "填报失败，请重试"
...
if __name__ == '__main__':
    ...
    for account in accounts:
        ...
        # 若当前步骤失败，则不在往下进行
        r1 = login_web(account["xh"], account["pwd"], web_url)
        if r1:
            r2 = get_private_data()
            if r2:
                submit_healthcondition()
        print(result)
