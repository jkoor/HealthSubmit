...
def email_result(result, submit_data, email, s_time):
    """发送填报结果至邮箱"""
    ...
    submit_info = ["您今天体温是否正常? : 是", "您今日是否处于中高风险地区? : 否", "和您共同居住的家庭成员是否有新冠肺炎确诊或疑似病例? : 否", "您是否正在进行集中隔离? : 否", "您是否今日解除集中隔离? : 否", "您是否正在进行居家隔离? : 否", "您今日是否进行了核酸检测? : 否", "您今日是否发生了所在地（跨地级市）异动变化? : 否", "您今日是否是确诊病例密切接触者? : 否", "您今日是否确诊为新冠肺炎病例? : 否", "您今日是否处于重点管控地区? : 否", "您今日是否处于重点关注地区? : 否"]
    msg = ["------------------------------", "填报时间 : " + s_time, "以上信息为系统自动发送，请勿回复", "若需取消自动填报或有其他问题可以发邮件至kiritor@qq.com", "---by jkor---"]
